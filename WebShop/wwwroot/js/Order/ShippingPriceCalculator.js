...
class ShippingPriceCalculator {
    ...
    async setShippingPrice(event) {
        const select = event.target;
        const carrierId = select.value;
        const carrier = await this.fetchCarrier(carrierId);
        let price = 0;
        if (carrier != null) {
...
//# sourceMappingURL=ShippingPriceCalculator.js.map